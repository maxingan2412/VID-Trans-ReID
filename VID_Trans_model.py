...
import copy
from vit_ID import TransReID,Block,TransReIDVideo
...
class VID_Trans(nn.Module):
    ...
    def forward(self, x, label=None, cam_label= None, view_label=None):  # label is unused if self.cos_layer == 'no'
        ...
        # for i in range(t):
        #     xt = x[:,i]
        #     cam_labelt = cam_label[::t]
        #     y = self.base(xt, cam_label=cam_labelt)  # 假设y的尺寸为[128, 129, 768]
        #     concat_features.append(y)
        # features = torch.cat(concat_features, dim=0)
...
class VID_TransVideo(nn.Module):
    def __init__(self, num_classes, camera_num, pretrainpath,seq_len):
        super(VID_TransVideo, self).__init__()
        self.in_planes = 768
        self.num_classes = num_classes
        self.seq_len=seq_len

        self.base = TransReIDVideo(
            img_size=[256, 128], patch_size=16, stride_size=[16, 16], embed_dim=768, depth=12, num_heads=12,
        ...
        self.bottleneck = nn.BatchNorm1d(
            self.in_planes)  # BatchNorm1d(768, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)
        self.bottleneck.bias.requires_grad_(
            False)  # 在这个例子中，self.bottleneck 是一个 nn.BatchNorm1d 层，它包括一个偏置（bias）参数。通过 self.bottleneck.bias.requires_grad_(False)，作者将这个偏置参数的梯度计算设置为不需要进行梯度更新,但是weigth是需要更新的
        self.bottleneck.apply(weights_init_kaiming)
        self.classifier = nn.Linear(self.in_planes, self.num_classes,
                                    bias=False)  # Linear(in_features=768, out_features=625, bias=False)
        self.classifier.apply(weights_init_classifier)
