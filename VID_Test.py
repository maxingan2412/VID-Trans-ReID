...
def evaluate(distmat, q_pids, g_pids, q_camids, g_camids, max_rank=21):
    ...
    indices = torch.argsort(distmat, axis=1)
    matches = (g_pids[indices] == q_pids[:, None]).to(torch.int32)
    ...
    for q_idx in range(num_q):
        ...
        tmp_cmc = torch.tensor(tmp_cmc) * orig_cmc
        AP = tmp_cmc.sum() / num_rel
        all_AP.append(AP)

    assert num_valid_q > 0, "Error: all query identities do not appear in gallery"

    all_cmc = torch.tensor(all_cmc).astype(torch.float32)
    all_cmc = all_cmc.sum(0) / num_valid_q
    mAP = torch.mean(all_AP)
...
def test(model, queryloader, galleryloader, pool='avg', use_gpu=True, ranks=[1, 5, 10, 20]):
    ...
    with torch.no_grad():
        for batch_idx, (imgs, pids, camids, _) in enumerate(queryloader):
            ...
            features = features.data.to(device)
            qf.append(features)
...
if __name__ == '__main__':
    ...
    model = VID_Trans(num_classes=num_classes, camera_num=camera_num,
                      pretrainpath='/home/ma1/work/VID-Trans-ReID/jx_vit_base_p16_224-80ecf9dd.pth')
